package src 
{
	...
	public class Main extends MovieClip 
	{
		...
		private var scrollSpeed:Number = 1; // tweakpunt 1: "Snelheid waarmee de achtergrond voorbij komt."
		...
		{	
			...
			numEnemiesToSpawn = 2+Math.ceil(wave / 10);//tweakpunt 3: "Formule die bepaalt hoeveel vijanden er zijn per wave."
			...
			if (wave >= 10)//tweakpunt 5: "Vanaf welke wave komen er speciale gouden enemies?"
			{
				var num:Number = numEnemiesToSpawn / 10;//tweakpunt 6: "De hoeveelheid gouden enemies."
				spawnSplicers(num);
			}				
		}			
		...
		{					
			for (var i:int = 0; i < number; i++) 
			{
				...
				{					
					...
					e.xSpeed = -2 + Math.round(Math.random() * 4);	
		...
		{		
			for (var i:int = 0; i < number; i++) 
			{
				...
				while(e.xSpeed < 1 && e.xSpeed > -1)e.xSpeed = -6 + Math.round(Math.random() * 12);	
			...
			if (powerup != null)
			{
				...
				{	
					...
					if (plane.hitTestPoint(powerup.x, powerup.y, true))
					{
						...
						if(powerup.currentLabel=="double")plane.activateDouble(25); //tweakpunt 12: "Hoe lang duren de powerups?"
						if(powerup.currentLabel == "special") special = new Special(this, 4500); //tweakpunt 13: "Hoe lang duren de powerups?"
						if(powerup.currentLabel=="rapid")plane.activateRapid(25); //tweakpunt 14: "Hoe lang duren de powerups?"
						if (powerup.currentLabel == "45angle") plane.activate45Angle(30); //tweakpunt 15 - "Hoe lang duren de powerups?"
						if (powerup.currentLabel == "90angle") plane.activate90Angle(30); //tweakpunt 16 - "Hoe lang duren de powerups?"
		...
		{		
			...
			for (var i:int = 0; i < lasers.length; i++) {removeChild(lasers[i]); }
			for (i = 0; i < enemies.length; i++) {removeChild(enemies[i]); }			
			for (i = 0; i < wingmen.length; i++) {if(contains(wingmen[i]))removeChild(wingmen[i]); }			
			removeChild(combo)
			combo = null;
			removeChild(space);
			removeChild(ui);
